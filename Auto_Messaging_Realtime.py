...
def listen() -> None:
	print('auto_msg_realtime was listened')
...
def ui():
	...
	im_line_notify_send_button.click(button_line,
									 inputs=all_args,
									 outputs=[im_line_notify_history])
	im_telegram_send_button.click(button_telegram,
								  inputs=all_args,
								  outputs=[im_telegram_notify_history])
	im_discord_send_button.click(button_discord,
								 inputs=all_args,
								 outputs=[im_discord_notify_history])
	setting_timer_start.click(timer,
							  inputs=all_args,
							  outputs=[setting_history])
...
BLUE = '\033[94m'
ENDC = '\033[0m'
print(f"{BLUE}[O][20241101][Extension][decadetw.Auto_Messaging_Realtime]{ENDC}\n\n")
